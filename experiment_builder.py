...
class ExperimentBuilder(object):
    def __init__(self, args, data, model, device):
        ...
        self.total_losses = {}
        self.state = {'best_val_r2': 0.0, 'best_val_iter': 0, 'current_iter': 0}
        self.start_epoch = 0
    ...
    def build_loss_summary_string(self, summary_losses):
        ...
        for key, value in zip(list(summary_losses.keys()), list(summary_losses.values())):
            if "loss" in key or "R_squared" in key:
                value = float(value)
    ...
    def save_val_models(self, model, epoch, state):
        """
        Saves the best model based on validation performance.

        - val_model_epoch_{epoch}: the best validation model at this epoch
        - val_model_best: always holds the latest best validation model
        """
        model.save_model(
            model_save_dir=os.path.join(self.saved_models_filepath, f"val_model_epoch_{int(epoch)}"),
            state=state,
        )

        model.save_model(
            model_save_dir=os.path.join(self.saved_models_filepath, "val_model_best"),
            state=state,
        )

        print("Saved best validation model to", self.saved_models_filepath)
    ...
    def evaluated_test_set_using_the_best_models(self, top_n_models):
        ...
        val_r2 = np.copy(per_epoch_statistics['val_R_squared_mean'])
        val_idx = np.array([i for i in range(len(val_r2))])
        sorted_idx = np.argsort(val_r2, axis=0).astype(dtype=np.int32)[::-1][:top_n_models]

        sorted_val_r2 = val_r2[sorted_idx]
        ...
        per_batch_targets = np.array(per_model_per_batch_targets[0]).reshape(per_batch_preds.shape)
        #print(per_batch_max)
        
        
        y_true = per_batch_targets.flatten()
        y_pred = per_batch_preds.flatten()  # Averaging predictions from ensemble
                ...
                for train_sample_idx, train_sample in enumerate(
                    ...
                    if self.state['current_iter'] % self.args.total_iter_per_epoch == 0:
                        ...
                        with tqdm.tqdm(total=int(self.args.num_evaluation_tasks // self.args.batch_size)) as pbar_val:
                            ...
                            for val_sample00, val_sample in enumerate(
                            ...
                            if (1 - val_losses["val_R_squared_mean"]) < self.state['best_val_r2']:
                                print("Best validation R_squared", val_losses["val_R_squared_mean"])
                                self.state['best_val_r2'] = val_losses["val_R_squared_mean"]
                                ...
                                self.save_val_models(model=self.model, epoch=self.epoch, state=self.state)
