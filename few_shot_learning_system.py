...
class MAMLFewShotRegressor(nn.Module):
    def __init__(self, im_shape, device, args):
        ...
        """
        super(MAMLFewShotRegressor, self).__init__()
        self.args = args
        self.device = device
        ...
        self.regressor = VGGReLUNormNetwork(im_shape=self.im_shape, 
                                             args=args, device=device, meta_regressor=True).to(device=self.device)
        ...
        # Optional multi-GPU
        if torch.cuda.is_available() and torch.cuda.device_count() > 1 and self.args.num_of_gpus > 1:
    ...
    def apply_inner_loop_update(self, loss, names_weights_copy, use_second_order, current_step_idx):
        ...
        if num_gpus > 1:
            self.regressor.module.zero_grad(params=names_weights_copy)
        else:
            self.regressor.zero_grad(params=names_weights_copy)
        ...
        names_weights_copy = {
            name.replace('module.', ''): value.unsqueeze(0).repeat(
                [num_devices] + [1 for i in range(len(value.shape))]).clone().detach().requires_grad_(True)
    ...
    def get_across_task_loss_metrics(self, total_losses, total_R_squared):
        losses = {
            'loss':      torch.mean(torch.stack(total_losses)),
            'R_squared': float(np.mean(total_R_squared)),
        }
    ...
    def forward(self, data_batch, epoch, use_second_order, use_multi_step_loss_optimization, num_steps, training_phase):
        ...
        for task_id, (x_support_set_task, y_support_set_task, x_target_set_task, y_target_set_task) in enumerate(zip(x_support_set,
            ...
            y_true = y_target_set_task.view(-1).float()
            y_pred = target_preds.view(-1).float()


            
            per_task_R_squared = r2_score(y_true.detach().cpu().numpy(), y_pred.detach().cpu().numpy())

            per_task_losses = torch.sum(torch.stack(task_losses))
            total_losses.append(per_task_losses)
            total_R_squared.append(per_task_R_squared.item())

            if not training_phase:
            
                _unwrap(self.regressor).restore_backup_stats()
    ...
    def net_forward(self, x, y, weights, backup_running_statistics, training, num_step):
        ...
        :param y: A data targets batch of shape (b, 1)
        ...
        loss1 = F.mse_loss(preds.view(-1), y.view(-1).float())
        

        r = y-preds
        loss = 2 * ((r - r.mean())*2).mean() + loss1
    ...
    def net_forward_finetune(self, x, y, weights, backup_running_statistics, training, num_step):
        """
        A base model forward pass on some data points x. Using the parameters in the weights dictionary. Also requires
        boolean flags indicating whether to reset the running statistics at the end of the run (if at evaluation phase).
        A flag indicating whether this is the training session and an int indicating the current step's number in the
    ...
    def run_train_iter(self, data_batch, epoch):
        ...
        y_support_set = torch.tensor(y_support_set, dtype=torch.float32, device=self.device)
        y_target_set = torch.tensor(y_target_set, dtype=torch.float32, device=self.device)'''
        
        x_support_set = x_support_set.clone().detach().float().to(self.device)
        x_target_set = x_target_set.clone().detach().float().to(self.device)
        y_support_set = y_support_set.clone().detach().float().to(self.device)
        y_target_set = y_target_set.clone().detach().float().to(self.device)
    ...
    def load_model_prime(self, model_save_dir, model_name, model_idx, device):
        """
        Load checkpoint and return the state dictionary.
        """
